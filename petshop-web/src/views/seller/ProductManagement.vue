  ...
  <div class="product-management">
    ...
    >
      <el-form :model="editingPet" label-width="100px">
        ...
        <el-form-item label="性别" prop="gender">
          <el-radio-group v-model="editingPet.gender">
...
// 数据获取
  const fetchPets = async () => {
    try {
      ...
      // 使用 apiClient，路径简化为 '/pets'（baseURL 已包含 '/api'）
...
const handleDeleteConfirm = (id) => {
  ElMessageBox.confirm('此操作将永久删除该宠物，是否继续？', '警告', {
    confirmButtonText: '确认删除',
    cancelButtonText: '取消操作',
    type: 'warning',
    center: true,          // 居中显示
    distinguishCancel: true // 区分取消和关闭
  })
      .then(() => {
        // 确认删除逻辑
        handleDelete(id)
...
// 删除（仅替换为 apiClient）
  const handleDelete = async (id) => {
    try {
      ...
      ElMessage.error(`存在关联订单，无法删除: ${err.response?.data?.message || err.message}`)
    }
  }
